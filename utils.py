...
def create_model(session, Model_class, path, load_vec, config, id_to_char, logger):
    ...
    model = Model_class(config)#config 是个有序的字典，这里调用init方法  一个字就是考科一个图，计算量大

    ckpt = tf.train.get_checkpoint_state(path)
    #通过checkpoint文件找到模型文件名 model_checkpoint_path: "ckpt\\ner.ckpt"   all_model_checkpoint_paths: "ckpt\\ner.ckpt"
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):  #如果没有训练好的模型，调转到训练模型开始训练
        ...
        if config["pre_emb"]:
            emb_weights = session.run(model.char_lookup.read_value())#字向量加载，参数加载到对应的模型
